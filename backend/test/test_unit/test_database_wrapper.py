...
class AddCameraToFilterTest(BaseTestCases.FilterTest):
    def test_existing_fid(self):
        """
        Test adding a camera to a filter.
        """
    ...
    def test_nonexistent_fid(self):
        """
        Test adding a camera to a nonexistent filter.
        """
        cm = get_camera_by_location(latitude=Decimal(value="13.37"), longitude=Decimal(value="0.42"))
        self.assertRaises(AssertionError, lambda: add_camera_to_filter(999, cmid=cm.id))

    def test_nonexistent_cid(self):
        """
        Test adding a nonexistent camera to a filter.
        """
        self.assertRaises(AssertionError, lambda: add_camera_to_filter(fid=self.fid, cmid=999))

    def test_add_multiple_cameras(self):
        """
        Test adding multiple cameras to a filter.
        """
        cm = get_camera_by_location(latitude=Decimal(value="13.37"), longitude=Decimal(value="0.42"))
        self.lat = Decimal(value="87.6")
        self.lon = Decimal(value="6.54")
        self.st = timezone.now() - timezone.timedelta(hours=1)
        self.et = timezone.now()
        self.fid = create_root_folder(path="/home/user/", name="test_folder")
        self.cid2 = create_clip(fid=self.fid, name="test_clip2", video_format="tvf", start_time=self.st,
                               end_time=self.et, latitude=self.lat, longitude=self.lon, width=256, height=240,
                               frame_rate=42.0)
        cm2 = get_camera_by_location(latitude=Decimal(value="87.6"), longitude=Decimal(value="6.54"))
        add_camera_to_filter(fid=self.fid, cmid=cm.id)
        add_camera_to_filter(fid=self.fid, cmid=cm2.id)

        self.assertEqual(len(get_all_cameras_in_filter(fid=self.fid)), 2)
...
class RemoveCameraFromFilterTest(BaseTestCases.FilterTest):
    ...
    def test_nonexistent_fid(self):
        """
        Test adding and removing a camera from a nonexistent filter.
        """
        cm = get_camera_by_location(latitude=Decimal(value="13.37"), longitude=Decimal(value="0.42"))
        add_camera_to_filter(fid=self.fid, cmid=cm.id)
        self.assertRaises(AssertionError, lambda:remove_camera_from_filter(fid=999, cmid=cm.id))

    def test_nonexistent_cmid(self):
        """
        Test adding and removing a nonexistent camera from a filter.
        """
        cm = get_camera_by_location(latitude=Decimal(value="13.37"), longitude=Decimal(value="0.42"))
        add_camera_to_filter(fid=self.fid, cmid=cm.id)
        self.assertRaises(AssertionError, lambda:remove_camera_from_filter(fid=self.fid, cmid=999))
