...
class ModifyFilterTest(TestCase):
    ...
    def test_simple_call(self):
        ...
        data = {FILTER_ID: self.fid,
                ...
                MIN_HEIGHT: 100,
                MIN_FRAMERATE: 10,
                ADD_BLACKLISTED_RESOLUTIONS: [[240, 256]],
                }
        ...
        self.assertEqual(len(filter.blacklisted_resolutions.filter(height=240, width=256)), 1)
...
class GetClipsMatchingFilter(TestCase):
    ...
    def setUp(self) -> None:
        ...
        dbw.create_clip(fid=self.rid, name="test_clip1", video_format="tvf",
                        start_time=self.st1, end_time=self.et1, latitude=self.lat,
                        longitude=self.lon, width=200, height=300, frame_rate=42.0)
